...
function contractIcon(typePoste?: string) {
  ...
  if (value.includes("cdi")) return "◻";
  if (value.includes("cdd")) return "◫";
  if (value.includes("stage")) return "△";
  if (value.includes("alternance")) return "◆";
  return "◇";
}
    ...
    <div className="listings-dashboard">
      ...
      {offresFiltrees.length === 0 ? (
        <section className="empty-state">
          ...
          <p>{entrepriseNom}, start by creating your first job listing.</p>
          <Button onClick={() => setShowCreateModal(true)}>Create my first role</Button>
        </section>
      ) : (
        ...
        <div className="modal-overlay">
          <div className="modal-card">
            <div className="modal-head">
              ...
              <button onClick={() => setShowCreateModal(false)} type="button">×</button>
            </div>
      ...
      <style jsx>{`
        .listings-dashboard {
          background: linear-gradient(180deg, #fbf8ff 0%, #f4f1fb 100%);
          border-radius: 24px;
          padding: 10px;
        ...
        .role-card {
          ...
          align-items: center;
          gap: 12px;
          box-shadow: 0 10px 20px rgba(53, 6, 62, 0.04);
        }
        .role-left {
          display: grid;
          gap: 8px;
